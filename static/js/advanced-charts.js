...
function initializeDeveloperCountryStudioHeatmap() {
    try {
        ...
        if (!rawData || rawData.length === 0) {
            ...
            ctx.font = '16px Arial';
            ctx.fillText('No country-studio data available', 50, 150);
            return;
        }
        ...
        createGroupedBarChart(canvas.getContext('2d'), canvas, countries, studioTypes, dataLookup, rawData);
    } catch (err) {
        console.error('Error initializing Country × Studio Type Matrix:', err);
    }
}
